...
function Sidebar({
    ...
    const avatar = user?.name ? user.name.split(" ").map((n) => [0]).join("").toUpperCase().slice(0, 2) : "U"
        ...
        <div className='flex flex-col h-full'>
                ...
                {sidebarOpen && (
                    <div className='flex items-center gap-2.5'>
                        ...
                        <motion.span
                            ...
                            className='font-extrabold text-sm tracking-tight text-[#0A0A0A] whitespace-nowrap'>SmarterAI</motion.span>
                    </div>
                )}
            ...
            <div className='px-2.5 pt-3 pb-1.5 shrink-0'>
                <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className={`w-full flex items-center gap-2 bg-[#000000] text-white font-semibold rounded-lg py-2 transition-all shadow-[1_4px_14px_rgba(0,0,0,0.25)] hover:bg-[#1a1a1a] ${sidebarOpen ? "px-2.5 " : "justify-center px-0"} cursor-pointer`}>
                ...
                <AnimatePresence>
                    {sidebarOpen && <motion.div
                        ...
                        <div className='flex items-center gap-1.5'>
                            <GiTwoCoins size={13} className='text-yellow-500 shrink-0' />
                            <div className='flex flex-col'>
                                <span className='text-[9px] uppercase tracking-wider text-white/40 font-medium'>Interview Coins</span>
                                <span className='text-xs font-bold text-white'>{user?.interviewCoins
                                }</span>
                            </div>
                        </div>
