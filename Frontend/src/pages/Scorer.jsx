...
const ScoreRing = ({ score }) => {
  const color = score >= 75 ? "#008000" : score >= 50 ? "#f59e0b" : "#ef4444";
  return (
...
const Tag = ({text, color}) => {
  const styles = {
    purple: "bg-purple-50    text-purple-700 border-puple-200",
    red:    "bg-red-50       text-red-700    border-red-200",
    green:  "bg-green-50     text-green-700  border-green-200",
    yellow: "bg-yellow-50    text-yellow-700 border-yellow-200",
  };
  return(
    <div className={`text-[10px] px-1.5 py-1 rounded-md border font-meedium ${styles [color]}`}>
      {text}
    </div>
  )
}
...
const Navbar = ({ label }) => {
  ...
  return (
    <motion.nav
      ...
      className='fixed inset-x-0 top-0 z-20 border-b border-black/8 bg-white/8 backdrop-blur-xl'>
        ...
        <motion.div
          ...
          <div className='relative'>
            ...
            <div className='flex items-center gap-1.5'>
              ...
              <span className='text-xs text-green-300'>{resume?.suggestedRole}</span>
            </div>
          </div>
    ...
    <div className='min-h-screen bg-white text-[#0A0A0A]'>
      ...
      <section className='flex min-h-screen items-center justify-center px-3 pt-18 pb-6'>
        ...
        <motion.div
          ...
          <div className='relative w-full h-1 bg-white/10 rounded-full mb-4'><div className='h-1 bg-white rounded-full w-1/2'
          /></div>
          ...
          <label className={`relative flex flex-col items-center justify-center w-full h-40 sm:h-48 rounded-2xl border-2 border-dashed cursor-pointer
            transition-colors
            ${file ? "border-white/40 bg-white/[0.06]"
              : "border-white/15 bg-white/[0.03] hover:border-white/30"
            }`}>
            <FiUploadCloud className={`text-4xl sm:text-5xl mb-2.5 ${file ? "text-white" : "text-white/30"}`} />
            <p className='text-xs font-medium text-white/80 '>{file ? file.name : "Click or drag PDF here"}</p>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={uploadedResume}
            disabled={!file} className='relative mt-4 w-full h-10 rounded-xl font-semibold text-xs bg-white text-[#0A0A0A] shadow-[0_4px_14px_rgba(255,255,255,0.15)] hover:bg-white/90 disabled:opacity-40 disabled:cursor-not-allowed transition-all'>
